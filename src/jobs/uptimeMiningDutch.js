...
function algoFromCoin(coin) {
  ...
  return ""; // fallback: vamos tentar por moeda
}

function buildCandidateUrls({ coin, account_id, api_key }) {
  const base = "https://www.mining-dutch.nl";
  const algo = algoFromCoin(coin);
  const coinLower = low(coin);
  const q = (name) =>
    `${base}/pools/${name}.php?page=api&action=getuserworkers&id=${encodeURIComponent(account_id)}&api_key=${encodeURIComponent(api_key)}`;

  const urls = [];
  if (algo) urls.push(q(algo));           // p.ex. /pools/scrypt.php
  if (coinLower) urls.push(q(coinLower)); // p.ex. /pools/litecoin.php
  // fallback hardening: tenta os dois algos comuns se nada foi definido
  if (!algo && !coinLower) urls.push(q("scrypt"), q("sha256"));
  return urls;
}
...
async function fetchMiningDutchWorkers({ coin, account_id, api_key }) {
  ...
  for (const url of urls) {
    try {
      ...
      // formatos possíveis:
      // { data: { workers: { "<name>": {hashrate,status,...}, ... } } }
      // ou { workers: {...} } ou array
      const workersNode =
        (data && (data.data?.workers || data.workers || data.data)) || null;

      let entries = [];
      if (workersNode && typeof workersNode === "object" && !Array.isArray(workersNode)) {
        entries = Object.entries(workersNode);
      } else if (Array.isArray(workersNode)) {
        entries = workersNode.map((v, i) => [String(v?.name ?? v?.worker ?? i), v]);
      } else {
        // schema estranho – tenta próximo URL
        continue;
      }
