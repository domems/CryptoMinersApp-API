...
export function startMonthlyClose() {
  ...
  cron.schedule("5 0 1 * *", async () => {
    ...
    try {
      // agrupa miners por utilizador
      const miners = await sql/*sql*/`
        SELECT id, user_id, nome, COALESCE(total_horas_online,0) AS hours,
               COALESCE(consumo_kw_hora,0) AS consumo_kw_hora,
               COALESCE(preco_kw,0) AS preco_kw
        FROM miners
      `;

      // criar/obter invoice por user
      ...
      for (const [userId, list] of byUser.entries()) {
        // cria header se não existir (idempotente)
        ...
        for (const m of list) {
          ...
          await sql/*sql*/`
            ...
            VALUES
              (${inv.id}, ${m.id}, ${m.nome || `Miner#${m.id}`}, ${hours}, ${kwh}, ${m.preco_kw}, ${m.consumo_kw_hora}, ${amount})
